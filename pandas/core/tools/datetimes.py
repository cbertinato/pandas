...
def _assemble_from_unit_mappings(arg, errors):
    ...
    if len(req):
        raise ValueError("to assemble mappings requires at "
                         ...
                         "[{miss}] is missing".format(miss=','.join(req)))
    ...
    if len(excess):
        raise ValueError("extra keys have been passed "
                         ...
                         "[{extra}]".format(extra=','.join(excess)))
        ...
        if format is not None:
            for element in arg:
                try:
                    ...
                    if errors == 'raise':
                        raise ValueError("Cannot convert {el} to a time with "
                                         "given format {fmt}"
                                         .format(el=element, fmt=format))
                    elif errors == 'ignore':
                        return arg
                    else:
                        times.append(None)
...
def ole2datetime(oledt):
    ...
    if val < 61:
        raise ValueError("Value is outside of acceptable range: {val} "
                         .format(val=val))
