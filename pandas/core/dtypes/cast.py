...
def astype_nansafe(arr, dtype, copy=True):
    ...
    if issubclass(dtype.type, text_type):
        ...
        if dtype == object:
            ...
            raise TypeError(
                "cannot astype a datetimelike from [{type1}] to [{type2}]"
                .format(type1=arr.dtype, type2=dtype))
        ...
        # in py3, timedelta64[ns] are int64
            ...
            raise TypeError(
                "cannot astype a timedelta from [{type1}] to [{type2}]"
                .format(type1=arr.dtype, type2=dtype))
...
def maybe_cast_to_datetime(value, dtype, errors='raise'):
    ...
    if dtype is not None:
        ...
        # coerce datetimelike to object
            ...
            raise TypeError(
                'Cannot cast datetime64 to {dtype}'.format(dtype=dtype))
