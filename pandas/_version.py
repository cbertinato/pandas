...
def run_command(commands, args, cwd=None, verbose=False, hide_stderr=False):
    ...
    for c in commands:
        try:
            ...
            if e.errno == errno.ENOENT:
                continue
            if verbose:
                print("unable to run {cmd}".format(cmd=dispcmd))
                print(e)
            return None
    else:
        if verbose:
            print("unable to find command, tried {cmd}".format(cmd=commands,))
        return None
    ...
    if p.returncode != 0:
        if verbose:
            print("unable to run {cmd} (error)".format(cmd=dispcmd))
        return None
    return stdout
...
def git_versions_from_keywords(keywords, tag_prefix, verbose):
    ...
    for ref in sorted(tags):
        ...
        if ref.startswith(tag_prefix):
            ...
            if verbose:
                print("picking {ref}".format(ref=r))
            return {"version": r,
...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    if "-" in git_describe:
        ...
        if not full_tag.startswith(tag_prefix):
            if verbose:
                fmt = "tag '{tag}' doesn't start with prefix '{prefix}'"
...
def render_pep440(pieces):
    ...
    if pieces["closest-tag"]:
        ...
        if pieces["distance"] or pieces["dirty"]:
            ...
            rendered += "{dist:d}.g{short}".format(dist=pieces["distance"],
                                                   short=pieces["short"])
            if pieces["dirty"]:
                rendered += ".dirty"
    else:
...
def render_pep440_pre(pieces):
    ...
    if pieces["closest-tag"]:
        rendered = pieces["closest-tag"]
        if pieces["distance"]:
            rendered += ".post.dev{dist:d}".format(dist=pieces["distance"])
    else:
        # exception #1
        rendered = "0.post.dev{dist:d}".format(dist=pieces["distance"])
    return rendered
...
def render_pep440_post(pieces):
    ...
    if pieces["closest-tag"]:
        rendered = pieces["closest-tag"]
        if pieces["distance"] or pieces["dirty"]:
            rendered += ".post{dist:d}".format(dist=pieces["distance"])
            if pieces["dirty"]:
                rendered += ".dev0"
            rendered += plus_or_dot(pieces)
            rendered += "g{dist}".format(dist=pieces["short"])
    else:
        # exception #1
        rendered = "0.post{dist:d}".format(dist=pieces["distance"])
        if pieces["dirty"]:
            rendered += ".dev0"
        rendered += "+g{dist}".format(dist=pieces["short"])
    return rendered
