...
def get_writer(engine_name):
    ...
    try:
        ...
        raise ValueError("No Excel writer '{eng}'".format(eng=engine_name))
...
class ExcelFile(object):
    ...
    def __init__(self, io, **kwds):
        ...
        if engine is not None and engine != 'xlrd':
            raise ValueError("Unknown engine: {eng}".format(eng=engine))
...
class ExcelWriter(object):
    ...
    def check_extension(cls, ext):
        ...
        if not any(ext in extension for extension in cls.supported_extensions):
            msg = (u("Invalid extension for engine '{eng}': '{ext}'")
                   .format(eng=pprint_thing(cls.engine),
    ...
    def __init__(self, path, engine=None, **engine_kwargs):
        if not openpyxl_compat.is_compat(major_ver=self.openpyxl_majorver):
            raise ValueError('Installed openpyxl is not supported at this '
                             'time. Use {ver}.x.y.'
                             .format(ver=self.openpyxl_majorver))
    ...
    def write_cells(self, cells, sheet_name=None, startrow=0, startcol=0,
        ...
        for cell in cells:
            ...
            if cell.mergestart is not None and cell.mergeend is not None:
                ...
                wks.merge_cells('{letterstart}{rowstart}:{letterend}{rowend}'
                                .format(letterstart=cletterstart,
                                        rowstart=startrow + cell.row + 1,
                                        letterend=cletterend,
                                        rowend=startrow + cell.mergestart + 1))
